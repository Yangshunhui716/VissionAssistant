...
export const analyzeCameraQuality = (buffer, width, height, bytesPerRow) => {
  ...
  const endX = Math.floor(width * 0.8) - STEP;
  const startY = Math.floor(height * 0.2);
  const endY = Math.floor(height * 0.8) - STEP;
  ...
  if (mean > THRESHOLD_GLARE) return { isBad: true, reason: 'Camera bị chói lóa' };

  if (variance < THRESHOLD_BLOCKED) return { isBad: true, reason: 'Camera đang bị che' };

  if (edgeStrength < THRESHOLD_BLUR && variance < 120) return { isBad: true, reason: 'Camera bị nhòe mất nét' };
